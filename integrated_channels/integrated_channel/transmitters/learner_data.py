...
from integrated_channels.utils import is_already_transmitted, generate_formatted_log
...
class LearnerTransmitter(Transmitter):
    ...
    def generate_common_params(self, **kwargs):
    ...
    def single_learner_assessment_grade_transmit(self, exporter, **kwargs):
        ...
        """
        app_label, enterprise_customer_uuid, lms_user_id = self.generate_common_params(**kwargs)
        ...
        for learner_data in exporter.single_assessment_level_export(**kwargs):
            ...
            try:
                ...
                self.handle_transmission_error(learner_data, client_error, app_label, enterprise_customer_uuid, lms_user_id, learner_data.course_id)

            except Exception:
                # Log additional data to help debug failures but just have Exception bubble
                self._log_exception_supplemental_data(
                    learner_data, 'create_assessment_reporting', app_label, enterprise_customer_uuid, lms_user_id, learner_data.course_id)
                raise
    ...
    def assessment_level_transmit(self, exporter, **kwargs):
        ...
        """
        app_label, enterprise_customer_uuid, _ = self.generate_common_params(**kwargs)
        TransmissionAudit = apps.get_model(  # pylint: disable=invalid-name
            app_label=app_label,
            model_name=kwargs.get('model_name', 'LearnerDataTransmissionAudit'),
        )
        ...
        for learner_data in exporter.bulk_assessment_level_export():
            ...
            lms_user_id = LearnerExporterUtility.get_lms_user_id_for_enterprise_course_enrollment_id(enterprise_enrollment_id)
    ...
    def _log_exception_supplemental_data(self, learner_data, operation_name, integrated_channel_name, enterprise_customer_uuid, learner_id, course_id):
        ...
        LOGGER.exception(generate_formatted_log(
            ...
            '{operation_name} failed with Exception for enterprise enrollment {enrollment_id} with payload {payload}'.format(
                operation_name=operation_name,
                enrollment_id=learner_data.enterprise_course_enrollment_id,
                payload=learner_data
            )), exc_info=True)

    def handle_transmission_error(self, learner_data, client_exception, integrated_channel_name, enterprise_customer_uuid, learner_id, course_id):
